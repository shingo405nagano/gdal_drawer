...
from typing import Any, Callable, Dict, Generator, List, NamedTuple, Optional, Union
...
class CustomGdalDataset(object):
    ...
    def _nodata_to(
        self, ary: np.ndarray, in_nodata: Any, out_nodata: Any
    ) -> gdal.Dataset:
    ...
    def expansion_dst(
        ...
        if 2 < len(ary.shape):
            ...
            count = 1
        kwargs = {"transform": transform, "projection": self.GetProjection()}
    ...
    def reprojected_bounds(
        ...
        xs, ys = gdal_utils.reproject_xy(
            [bounds.x_min, bounds.x_max], [bounds.y_min, bounds.y_max], in_crs, out_crs
        )
    ...
    def _adjustment_of_length(
        self, geoms: gpd.GeoSeries, data: Dict[str, np.ndarray]
    ) -> Any:
        """
        ## Summary
            小数部の影響か、たまに範囲外の座標が生成されるので、範囲内のものだけを取得
    ...
    def estimate_utm_crs(self, **kwargs) -> str:
        ...
        if org_crs.to_epsg() != 4326:
            ...
            center = gdal_utils.reproject_xy(
                center.x, center.y, org_crs.to_wkt(), wgs_crs
            )
        # UTMのCRSを推定する
    ...
    def _resample_option_template_with_resol_spec(
        self,
        x_resolution: float,
        y_resolution: float,
    ...
    def _resample_option_temppate_with_cells_spec(
        self, x_cells: int, y_cells: int, resample_algorithm: int = gdal.GRA_CubicSpline
    ) -> gdal.WarpOptions:
        """
        ## Summary
        ...
        **kwargs,
        ...
        if kwargs.get("forced_metre_system", True) and not self.check_crs_is_metre():
            ...
            x_resolution = gdal_utils.degree_from_metre(
                x_resolution, center.x, center.y, wkt_crs, x_direction=True
            )
            # y_resolutionをMetreからDegree単位に変換
            y_resolution = gdal_utils.degree_from_metre(
                y_resolution, center.x, center.y, wkt_crs, x_direction=False
            )
    ...
    def _clip_option_template_with_wkt_poly_spec(
        self, wkt_poly: str, fmt: str = "MEM", nodata: Any = np.nan, **kwargs
    ) -> gdal.Dataset:
        """
        ## Summary
    ...
    def get_masked_array(
        ...
        gdal.RasterizeLayer(
            mask_dst, bands, mask_lyr, burn_values=[masked_value], options=ops
        )
    ...
    def outlier_treatment_array_by_std(self, sigma: float = 2, **kwargs) -> np.ndarray:
        """
        ## Summary
    ...
    def outlier_treatment_array_by_iqr(
        self, threshold: float = 1.5, **kwargs
    ) -> np.ndarray:
        """
        ## Summary
    ...
    def slope_with_distance_spec(
        ...
        kernel_size = kernels.distance_to_kernel_size(
            distance, x_cell_size, y_cell_size
        )
    ...
    def tpi(self, **kwargs) -> Union["CustomGdalDataset", gdal.Dataset]:
        ...
        if "kernel" in kwargs:
            ...
            if 1 <= nan_idx.size:
                ...
                ary[nan_idx] = 0
            conved_ary = scipy.signal.convolve(ary, kernel, mode="same", method="fft")
            if 1 <= nan_idx.size:
        ...
        if outlier_treatment:
            # 外れ値処理
            tpi_ary = self._outlier_treatment(ary=tpi_ary, threshold=outlier_treatment)
        if kwargs.get("return_array", False):
            # return_arrayが指定されている場合は、numpy配列で返す
            tpi_ary = tpi_ary.astype(np.float16)
            return tpi_ary
    ...
    def mean_kernel_from_distance(
        self, distance: float, metre: bool = True
    ) -> np.ndarray:
        """
        ## Summary
    ...
    def doughnut_kernel_from_distance(
        self, distance: float, metre: bool = True
    ) -> np.ndarray:
        """
        ## Summary
    ...
    def gaussian_kernel_from_distance(
        self, distance: float, metre: bool = True, coef: float = None
    ) -> np.ndarray:
        """
        ## Summary
    ...
    def inverse_gaussian_kernel_from_distance(
        self, distance: float, metre: bool = True, coef: float = None
    ) -> np.ndarray:
        """
        ## Summary
        ...
        if metre:
            ...
            y_resol = self.y_resolution
        kernel_size = kernels.distance_to_kernel_size(distance, x_resol, y_resol)
        return kernels.inverse_gaussian_kernel_from_size(
            kernel_size.x, kernel_size.y, coef
        )

    #############################################################################
    # ----------------- Methods for plotting. -----------------
    def plot_raster(self, fig: Figure, ax: Axes, **kwargs) -> None:
        """
        ## Summary
            ラスターデータをプロットする。
        Args:
